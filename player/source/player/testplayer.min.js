...
InteractiveTask.Player = function(options){
	...
	//  Ïîïûòêà çàãðóçèòü è ïåðåíàñòðîèòü êîíñòàíòû ïëååðà
	try{
...
/*Ìåòîä î÷èñòêè ïëååðà ïîñëå åãî îòðàáîòêè*/
...
InteractiveTask.Player.prototype.getOutResult = function(){
	...
	var maxScore = 0;
	var currentScore = 0;
	var cTaskResult = 0; // ðåçóëüòàò òîëüêî äëÿ òåêóùåãî çàäàíèÿ
	var taskID = 0;
	var deltaScore;
	var deltaTime;
...
/**
 * Âû÷èñëåíèå ðåçóëüòàòà äëÿ i-îãî çàäàíèÿ ñ ó÷åòîì ôàêòîðîâ âëèÿþùèõ íà ïîíèæåíèå áàëëà (ïîäâîäÿùèå, æèçíè, òàéìåð)
 * @param i
 * @returns {number}
 */
...
InteractiveTask.Player.prototype.prepareArrayTaskOptions = function(){
	...
	for(i=0;i<l;i++){
		...
		this.outResult[i] = {
			...
			isComplate : false,
			maximumResult : maxResult,  // âçÿòü äàííûé ðåçóëüòàò èç ôàéëà çàäàíèÿ (ðàçðàáîòàòü â êîíñòðóêòîðå óñòàíîâêó áàëëîâ)
			numHelping : 0, // óêàçûâàòü óðîâåíü ïîäâîäÿùåãî çàäàíèÿ äëÿ îïðåäåëåíèÿ ïîíèæàþùåãî áàëà
			comment : "",
...
// Çàâåðøåíèå çàäàíèÿ ïðè îøèáêå
// - Çàêîí÷èëñÿ òàéìåð
// - Çàêîí÷èëèñü æèçíè
// - Íàæàòà êíîïêà "Íå çíàþ" â òåñòå
...
// Çàâåðøåíèå çàäàíèÿ ãèáðèäíîãî ðåæèìà ïðè ïîâòîðíîé îøèáêå
// - çàêîí÷èëñÿ òàéìåð äâàæäû
// - çàêîí÷èëèñü îøèáêè äâàæäû
...
// Íàæàòà êíîïêà "Ðåñòàðò" èëè ïåðåçàãðóçêà çàäàíèÿ ïî äðóãîé ïðè÷èíå
...
// Åñòü ëè ó çàäàíèÿ ñâÿçàííîå àóäèî è äèñïàò÷èò â ñëó÷àå íåîáõîäèìîñòè
...
// Ïîëó÷åíèå ñëåäóþùåãî çàäàíèÿ ïðè ðàçëè÷íûõ ñîáûòèÿõ
...
// Ïîëó÷åíèå íîìåðà ñëåäóþùåãî çàäàíèÿ â äèôôåðåíöèðîâàííîì äåðåâå  ïðè ïîçèòèâíîì ïåðåõîäå:
// - óñïåøíîå çàâåðøåíèå
// - êíîïêà ÿ ïîíÿë
// - îøèáêà â ìíèìîì
...
// Ïîëó÷åíèå íîìåðà ñëåäóþùåãî çàäàíèÿ â ëèíåéíîì ñïèñêå
// âîçâðàùàåò -1 åñëè ñëåäóþùåãî íåò
// âîçâðàùàåò ID åñëè åñòü
...
//
/* ÁËÎÊ ÏÐÎÂÅÐÊÈ ÍÀËÈ×Èß ÏÐÈÊÐÅÏËÅÍÍÎÃÎ ÀÓÄÈÎ È ÂÎÇÂÐÀÙÅÍÈß ÍÎÌÅÐÎÂ ÔÀÉËÎÂ*/
//
...
InteractiveTask.Player.prototype.startCurrentTask = function(){
	...
	if(this.currentTaskID!=-1){
		//  ïåðåä íà÷àëîì çàäàíèÿ, â ñëó÷àå íåîáõîäèìîñòè ñíèæàåì áàëë çà ïîëó÷åííóþ ïîìîùü
		this.downScore();
...
/********************************************
 * óñòàíàâëèâàåì ñíèæàþùèé áàëë íà çàäíèå
 * @returns {*}
 */
...
/************************************************************************************
 /************************************************************************************
 /************************************************************************************
 *    ÁËÎÊ ÊÎÍÒÐÎËß ÑÌÅÍÛ ÇÀÄÀÍÈÉ    ÎÊÎÍ×ÅÍ
 ************************************************************************************/
/************************************************************************************/
/************************************************************************************/
/***********************************************************************
...
/**
 * Created by ÐÐ°ÑÐ° on 02.05.15.
 */
...
InteractiveTask.SampleMark = function(options){
	...
	if(this.xml.STARTANIMATIONCOMPLATE != undefined){
		//this.startLabelComplate = this.xml.STARTANIMATIONCOMPLATE;
		/*
		 Ð¼ÐµÐ½ÑÐµÐ¼ Ð½Ð° ÑÐ¾Ð·Ð´Ð°Ð½Ð¸Ðµ Ð¼Ð°ÑÑÐ¸Ð²Ð° Ð¼ÐµÑÐ¾Ðº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
		 */
	...
	//TODO: Ð²Ð²ÐµÑÑÐ¸ touchstart Ð¸ Ð¿ÑÐ¾Ð²ÐµÑÐ¸ÑÑ Ð½Ð° Ð¿Ð»Ð°Ð½ÑÐµÑÐ°Ñ
...
/*
 Ð£ÑÑÐ°Ð½Ð¾Ð²ÐºÐ° Ð¾Ð±Ð»Ð°ÑÑÐ¸ Ð²Ð½ÐµÑÐµÐ½Ð¸Ñ Ð² ÑÐ»ÑÑÐ°Ðµ ÐµÑÐ»Ð¸ ÑÑÐ¾ Ð½Ð¾Ð±ÑÐ¾Ð´Ð¸Ð¼Ð¾  ///////////////////////////////////////////////////////////////////////////////////
 */
...
/**
 * Ð?ÑÐ¿ÑÐ°Ð²Ð¸ÑÑ ÑÐ°Ð±Ð¾ÑÑ Ñ ÑÐ»Ð¾ÐµÐ¼
 * @param options
 * @constructor
 */
...
// Ð¡Ð¾Ð·Ð´Ð°ÑÐ¼ Ð´Ð²Ð° Ð¼Ð°ÑÑÐ¸Ð²Ð° ÐºÐ»ÐµÑÐ¾Ðº //////////////////////////////////////////////////////////////////////////////////////////
	for(i=0;i<numline;i++){
		net1[i] = [];
		net2[i] = [];
		idMas[i] = [];
...
// Ð¡Ð²Ð¾Ð¹ÑÑÐ²Ð° ÐºÐ°ÑÑÐ¸Ð½Ð¾Ðº ///////////////////////////////////////////////////////////////////////////////////////////////////
...
// ÐÑÐ¾Ð²ÐµÑÐºÐ°: Ð¸Ð·Ð¾Ð±ÑÐ°Ð¶ÐµÐ½Ð¸Ðµ Ð¸Ð»Ð¸ Ð¿ÑÑÑÐ¾ /////////////////////////////////////////////////////////////////////////////////////
...
// ÐÑÐ¾Ð²ÐµÑÐºÐ° Ð½Ð° ÑÐ»ÑÑÐ°Ð¹Ð½ÑÑ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ Ð²Ð¾Ð¿ÑÐ¾ÑÐ°/Ð¸Ð·Ð¾Ð±ÑÐ°Ð¶ÐµÐ½Ð¸Ñ ///////////////////////////////////////////////////////////////////
...
// ÐÑÐ¾Ð²ÐµÑÐºÐ°: Ð²Ð¾Ð¿ÑÐ¾Ñ Ð¸Ð»Ð¸ Ð¸Ð·Ð¾Ð±ÑÐ°Ð¶ÐµÐ½Ð¸Ðµ ////////////////////////////////////////////////////////////////////////////////////
	if(this.xml.QUESTION == "false"){
	...
	var self = this.controller;
// ÐÐ±ÑÐ°Ð±Ð¾ÑÐºÐ° ÐºÐ»Ð¸ÐºÐ° Ð¼ÑÑÐ¸ /////////////////////////////////////////////////////////////////////////////////////////////////
	for (i=0;i<numline;i++){
		for (j=0;j<numcolumn;j++){
...
/**
 * Ð?ÑÐ¿ÑÐ°Ð²Ð¸ÑÑ ÑÐ°Ð±Ð¾ÑÑ ÑÐ¾ ÑÐ»Ð¾ÑÐ¼Ð¸
 * @param options
 * @constructor
 */
...
// ÐÑÑÑ Ð´Ð¾ Ð¼ÐµÑÑÐ¾ÑÐ°ÑÐ¿Ð¾Ð»Ð¾Ð¶ÐµÐ½Ð¸Ñ ÐºÐ°ÑÑÐ¸Ð½ÐºÐ¸ + Ð½Ð°Ð·Ð²Ð°Ð½Ð¸Ðµ ÑÐ°Ð¼Ð¾Ð¹ ÐºÐ°ÑÑÐ¸Ð½ÐºÐ¸ ////////////////////////////////////////////////////////
...
// Ð¡Ð¾Ð·Ð´Ð°ÑÐ¼ Ð¼Ð°ÑÑÐ¸Ð² ÐºÐ»ÐµÑÐ¾Ðº Ð¸ Ð¿Ð°Ð·Ð»Ð¾Ð² //////////////////////////////////////////////////////////////////////////////////////
	for (i=0;i<numCol;i++){
		net[i] = [];
		pic[i] = [];
		for(j=0;j<numLine;j++){
...
// ÐÐ¾Ð±Ð°Ð²Ð»ÑÐµÐ¼ ÑÐ»ÐµÐ¼ÐµÐ½ÑÑ Ð½Ð° ÑÐ»Ð¾Ð¹ Ñ ÑÑÐµÑÐ¾Ð¼ ÑÐµÐ³Ð¾Ð² xml ///////////////////////////////////////////////////////////////////////
			net[i][j].id=id_net++;
			net[i][j].done=false;
...
// ÐÐ±ÑÐ°Ð±Ð¾ÑÐºÐ° Ð²Ð·Ð°Ð¸Ð¼Ð¾Ð´ÐµÐ¹ÑÑÐ²Ð¸Ð¹ Ñ Ð¿Ð°Ð·Ð»Ð¾Ð¼ (Ð²ÑÐ°ÑÐµÐ½Ð¸Ðµ, Ð¿ÐµÑÐµÐ¼ÐµÑÐµÐ½Ð¸Ðµ) ////////////////////////////////////////////////////////////
	var self = this.controller;
	for(n=0;n<numCol;n++){
		for(m=0;m<numLine;m++){
...
// ÐÐ±ÑÐ°Ð±Ð¾ÑÐºÐ° Ð¿Ð¾Ð¿Ð°Ð´Ð°Ð½Ð¸Ñ Ð¿Ð°Ð·Ð»Ð° Ð² ÐºÐ»ÐµÑÐºÑ Ð¸ Ð¿ÑÐ¾Ð²ÐµÑÐºÐ° Ð½Ð° Ð¿ÑÐ°Ð²Ð¸Ð»ÑÐ½Ð¾ÑÑÑ ///////////////////////////////////////////////////////
...
// Ð¤ÑÐ½ÐºÑÐ¸Ð¸ Ð¿Ð¾Ð»ÑÑÐµÐ½Ð¸Ñ Ð¿Ð¾Ð´ÑÐºÐ°Ð·ÐºÐ¸ Ð¸ Ð¿ÑÐ¾Ð²ÐµÑÐºÐ¸ ÑÐµÐ·ÑÐ»ÑÑÐ°ÑÐ° ///////////////////////////////////////////////////////////////////
...
//	InteractiveTask.log("this.colorTan.strokeAlpha() = " + this.colorTan.strokeAlpha());
	...
	if(this.xml.BLACK.DELETE == "1"){
		...
		/*ÑÐ²Ð¾Ð¹ÑÑÐ²Ð¾ Ð´Ð»Ñ ÑÑÐ½ÐºÑÐ¸Ð¸ Ð¿ÐµÑÐµÐ¼ÐµÑÐµÐ½Ð¸Ñ/Ð²ÑÐ°ÑÐµÐ½Ð¸Ñ Ð¾Ð±ÑÐµÐºÑÐ¾Ð² ÑÑÑÐ°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÑÑÑ Ð² true Ð´Ð»Ñ ÑÐ¾Ð³Ð¾ ÑÑÐ¾Ð±Ñ Ð²ÑÑÐ¸ÑÐ»ÑÐ»Ð°ÑÑ Ð¾Ð±Ð»Ð°ÑÑÑ ÐºÐµÑÐ¸ÑÐ¾Ð²Ð°Ð½Ð¸Ñ
		 *  ÑÐ¾Ð»ÑÐºÐ¾ Ð´Ð»Ñ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÑÑÐºÐ¸Ñ ÑÐ°Ð½Ð¾Ð²*/
...
InteractiveTask.SampleUserTan.prototype.colorableEnabled = function(){
	...
	try{    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if(this.xml.COLOR.PAINT == "1"){
			...
			// Ð½ÐµÑÐ²Ð½Ð°Ñ Ð¿ÑÐ¾Ð²ÐµÑÐºÐ° Ð½ÑÐ¶Ð½Ð¾Ð³Ð¾ ÑÐ²ÐµÑÐ° Ð½Ð° Ð´Ð¾ÑÐ¾Ð»ÑÐ¾Ð²Ð¾ÑÑÑ, ÐµÑÐ»Ð¸ Ð¾Ð½Ð¸ ÑÐ¾Ð²Ð¿Ð°Ð´Ð°ÑÑ, ÑÐ¾ ÑÐ°Ð½ ÑÐ°ÑÐºÑÐ°ÑÐµÐ½ Ð²ÐµÑÐ½Ð¾
...
//  ÐÐ¿ÑÐµÐ´ÐµÐ»ÑÐµÑ ÑÑÑÐ°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÑÑ Ð»Ð¸ ÑÐ°Ð½Ñ Ð½Ð°Ð·Ð°Ð´ Ð½Ð° ÑÐ²Ð¾Ñ Ð³Ð»ÑÐ±Ð¸Ð½Ñ Ð² ÑÐ»Ð¾Ðµ (Ð´Ð»Ñ Ð¿ÐµÑÐµÐ¼ÐµÑÐµÐ½Ð¸Ñ Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸)
...
//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð·Ð²ÑÐºÐ° Ð¿Ð¾ Ð²ÐµÑÑÐ¸ÐºÐ°Ð»Ð¸: top, center, bottom
InteractiveTask.Const.prototype.SOUND_VERTICAL_ALIGN = "top";
//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð·Ð²ÑÐºÐ° Ð¿Ð¾ Ð³Ð¾ÑÐ¸Ð·Ð¾Ð½ÑÐ°Ð»Ð¸: left, center, right
InteractiveTask.Const.prototype.SOUND_HORIZONTAL_ALIGN = "left";

//  ÐÑÑÑÑÐ¿ Ð¾Ñ Ð¾Ð¿ÑÐµÐ´ÐµÐ»ÐµÐ½Ð½Ð¾Ð³Ð¾ ÐºÑÐ°Ñ Ð¿Ð¾ Ð¾ÑÐ¸ Ð¥
InteractiveTask.Const.prototype.SOUND_DELTA_X = 70;
//  ÐÑÑÑÑÐ¿ Ð¾Ñ Ð¾Ð¿ÑÐµÐ´ÐµÐ»ÐµÐ½Ð½Ð¾Ð³Ð¾ ÐºÑÐ°Ñ Ð¿Ð¾ Ð¾ÑÐ¸ Y
InteractiveTask.Const.prototype.SOUND_DELTA_Y = 5;

//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð¿Ð¾Ð»Ð½Ð¾ÑÐºÑÐ°Ð½Ð½Ð¾Ð³Ð¾ ÑÐµÐ¶Ð¸Ð¼Ð° Ð¿Ð¾ Ð²ÐµÑÑÐ¸ÐºÐ°Ð»Ð¸: top, center, bottom
InteractiveTask.Const.prototype.FULLSCREEN_VERTICAL_ALIGN = "top";
//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð¿Ð¾Ð»Ð½Ð¾ÑÐºÑÐ°Ð½Ð½Ð¾Ð³Ð¾ ÑÐµÐ¶Ð¸Ð¼Ð° Ð¿Ð¾ Ð³Ð¾ÑÐ¸Ð·Ð¾Ð½ÑÐ°Ð»Ð¸: left, center, right
...
//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð¿Ð°ÑÐ·Ñ Ð¿Ð¾ Ð²ÐµÑÑÐ¸ÐºÐ°Ð»Ð¸: top, center, bottom
InteractiveTask.Const.prototype.PAUSE_VERTICAL_ALIGN = "top";
//  ÐÐ¾Ð¿ÑÑÑÐ¸Ð¼ÑÐµ ÐºÐ¾Ð½ÑÑÐ°Ð½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ°Ð²Ð½Ð¸Ð²Ð°Ð½Ð¸Ñ ÐºÐ½Ð¾Ð¿ÐºÐ¸ Ð¿Ð°ÑÐ·Ñ Ð¿Ð¾ Ð³Ð¾ÑÐ¸Ð·Ð¾Ð½ÑÐ°Ð»Ð¸: left, center, right
...
// ÐÑÐµÐ¼Ñ Ð¾ÑÐ¾Ð±ÑÐ°Ð¶ÐµÐ½Ð¸Ñ Ð¾ÐºÐ¾Ð½ Ð¿ÐµÑÐµÑÐ¾Ð´Ð¾Ð² Ð¼ÐµÐ¶Ð´Ñ Ð·Ð°Ð´Ð°Ð½Ð¸ÑÐ¼Ð¸ (ÑÑÐ¿ÐµÑ/Ð½ÐµÑÐ´Ð°ÑÐ°) Ð² Ð¼Ð¸Ð»Ð¸ÑÐµÐºÑÐ½Ð´Ð°Ñ
InteractiveTask.Const.prototype.SHOWING_FINAL_FRAME_TIME = 2000;

//  ÐÐ¾Ð½Ð¸Ð¶Ð°ÑÑÐ¸Ð¹ ÐºÐ¾ÑÑÑÐ¸ÑÐ¸ÐµÐ½Ñ Ð´Ð»Ñ Ð·Ð°Ð´Ð°Ð½Ð¸Ð¹
InteractiveTask.Const.prototype.DECREASING_COEFFICIENT = 0.9;

//  ÐÐ°ÑÐ°Ð¼ÐµÑÑ ÑÐ¸ÑÐ»Ð° ÐºÐ°Ð´ÑÐ¾Ð² Ð² ÑÐµÐºÑÐ½Ð´Ñ Ð´Ð»Ñ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ Ð¾Ð±ÑÐµÐºÑÐ¾Ð² (Ð»Ð¸Ð½ÐµÐ¹Ð½Ð°Ñ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ ÑÐ¾Ð·Ð´Ð°Ð½Ð½Ð°Ñ Ð² ÐºÐ¾Ð½ÑÑÑÑÐºÑÐ¾ÑÐµ)
InteractiveTask.Const.prototype.ANIMATION_FRAME_RATE = 24;

//  ÐÐ¾ÑÑÑÐ¸ÑÐ¸ÐµÐ½ÑÑ Ð´Ð»Ñ Ð²ÑÑÐ¸ÑÐ»ÐµÐ½Ð¸Ñ Ð´ÐµÐ»ÑÑÐ° Ð¾ÑÑÐµÐ·ÐºÐ¾Ð² ÑÐ°Ð¹Ð¼ÐµÑÐ°
InteractiveTask.Const.prototype.FULL_TIME_DIVIDER = 100;
InteractiveTask.Const.prototype.TIME_DIVIDER = 10;

/*ÐÐ°ÑÐ°Ð¼ÐµÑÑÑ ÑÐ¸Ð½ÑÐ¾Ð²*/
//  ÐÑÐ²Ð¾Ð´Ð¸ÑÑ Ð»Ð¸ ÑÐ¸Ð½ÑÑ
InteractiveTask.Const.prototype.IS_ADD_HINT = true;
//  Ð±Ð°Ð·Ð¾Ð²ÑÐµ Ð½Ð°ÑÑÑÐ¾Ð¹ÐºÐ¸ ÑÐ¸Ð½ÑÐ° opacity - Ð¿ÑÐ¾Ð·ÑÐ°ÑÐ½Ð¾ÑÑÑ ÑÐ¸Ð½ÑÐ°, visible - Ð²Ð¸Ð´Ð¸Ð¼ Ð»Ð¸ Ð¸Ð·Ð½Ð°ÑÐ°Ð»ÑÐ½Ð¾ ÑÐ¸Ð½Ñ, listening - Ð´Ð¾ÑÑÑÐ¿ÐµÐ½ Ð»Ð¸ Ð´Ð»Ñ Ð¿ÑÐ¾ÑÐ»ÑÑÐ¸Ð²Ð°Ð½Ð¸Ñ ÑÐ¾Ð±ÑÑÐ¸Ð¹
...
//  ÐÐ¸Ð·ÑÐ°Ð»Ð¸Ð·Ð°ÑÐ¸Ñ ÑÐ¾Ð½Ð° ÑÐ¸Ð½ÑÐ°
...
//  ÐÐ°ÑÑÑÐ¾Ð¹ÐºÐ¸ ÑÐµÐºÑÑÐ° ÑÐ¸Ð½ÑÐ°
...
//  Ð¢ÐµÐºÑÑÑ ÑÐ¸Ð½ÑÐ¾Ð² Ð´Ð»Ñ ÐºÐ½Ð¾Ð¿Ð¾Ðº
...
//  ÐÐ°Ð¿ÑÐ°Ð²Ð»ÐµÐ½Ð¸Ðµ ÑÑÑÐµÐ»ÐºÐ¸ Ð² ÑÐ¸Ð½ÑÐµ
...
//  ÐÐ½Ð¾Ð¿ÐºÐ° ÑÑÐ°ÑÑÐ° Ð·Ð°Ð´Ð°Ð½Ð¸Ñ Ð´Ð»Ñ Ð·Ð°Ð³ÑÑÐ·ÐºÐ¸ ÐºÐ¾Ð½ÑÐµÐ½ÑÐ° Ð² Ð±Ð¸Ð±Ð»Ð¸Ð¾ÑÐµÐºÑ Ð½Ð° Ð¼Ð¾Ð±Ð¸Ð»ÑÐ½ÑÑ Ð´ÐµÐ²Ð°Ð¹ÑÐ°Ñ
...
InteractiveTask.AnimationController = function(){
	...
	//  ÐÑÑÐµÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹ (Ð²Ð½Ð°ÑÐ°Ð»Ð° Ð¾Ð½Ð¸ Ð½Ð°ÐºÐ°Ð¿Ð»Ð¸Ð²Ð°ÑÑÑÑ Ð·Ð´ÐµÑÑ, Ð·Ð°ÑÐµÐ¼ Ð¿Ð¾ÑÑÑÐ¿Ð°ÑÑ Ð² Ð¼Ð°ÑÑÐ¸Ð² playAnimation, ÐµÑÐ»Ð¸ Ð¸Ñ Ð½ÑÐ¶Ð½Ð¾ Ð²Ð¾ÑÐ¿ÑÐ¾Ð¸Ð·Ð²Ð¾Ð´Ð¸ÑÑ)
	this.bufferAnimation = new Array();
	//  ÐÐ°ÑÑÐ¸Ð² Ð²Ð¾ÑÐ¿ÑÐ¾Ð¸Ð·Ð²Ð¾Ð´Ð¸Ð¼ÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹ Ð² Ð´Ð°Ð½Ð½ÑÐ¹ Ð¼Ð¾Ð¼ÐµÐ½Ñ
...
//  Ð¡ÑÐ°ÑÑ Ð°Ð²ÑÐ¾Ð¼Ð°ÑÐ¸ÑÐµÑÐºÐ¸Ñ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹ Ð² Ð½Ð°ÑÐ°Ð»Ðµ Ð·Ð°Ð´Ð°Ð½Ð¸Ñ
InteractiveTask.AnimationController.prototype.totalPlaye = function(){
	var i, l, n;
	l = this.bufferAnimation.length;
	var flag = false;
	for(i=0;i<l;i++){
		//  ÐÑÐ»Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ Ð½ÑÐ¶Ð½Ð¾ ÑÑÐ°ÑÑÐ¾Ð²Ð°ÑÑ Ð°Ð²ÑÐ¾Ð¼Ð°ÑÐ¸ÑÐµÑÐºÐ¸
		if(this.bufferAnimation[i].isAutoPlay()){
			flag = true;
			//  Ð Ð°ÑÑÑÐ¸ÑÑÐ²Ð°ÐµÐ¼ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÐµ ÑÐ¾ÑÐºÐ¸
			this.bufferAnimation[i].middlePointsAnimation();
			//  ÐÑÐ¿ÑÐ°Ð²Ð»ÑÐµÐ¼ Ð¾Ð±ÑÐµÐºÑ Ð² ÑÐ»Ð¾Ð¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
			this.bufferAnimation[i].moveToAnimationLayer();
			//  ÐÑÐ¿ÑÐ°Ð²Ð»ÑÐµÐ¼ Ð² Ð¼Ð°ÑÑÐ¸Ð² Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹ Ð¿ÑÐ¾Ð¸Ð³ÑÑÐ²Ð°Ð½Ð¸Ñ  Ð¸ ÐÑÑÐµÐ·Ð°ÐµÐ¼ ÐµÑ Ð¸Ð· Ð±ÑÑÐµÑÐ°
			...
			//  Ð¿Ð¾ÑÐºÐ¾Ð»ÑÐºÑ Ð¼Ñ Ð²ÑÑÐµÐ·Ð°Ð»Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ Ð¸Ð· Ð±ÑÑÐµÑÐ°, ÑÐ¾ Ð¿Ð¾Ð½Ð¸Ð¶Ð°ÐµÐ¼ Ð¸ Ð½Ð´ÐµÐºÑ Ð¸ Ð´Ð»Ð¸Ð½Ñ Ð¼Ð°ÑÑÐ¸Ð²Ð° Ð±ÑÑÐµÑÐ°
			--l;
			--i;
		};
	};
...
//	InteractiveTask.log("animation play");
	...
	var flag = false;
	if(l>0){
		//  ÐÐ¾ Ð²ÑÐµÐ¼ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸ÑÐ¼ Ð¿ÑÐ¾Ð¸Ð³ÑÑÐ²Ð°Ð½Ð¸Ñ Ð¿ÐµÑÐµÐ¼ÐµÑÐ°ÐµÐ¼ Ð¾Ð±ÑÐµÐºÑÑ Ð² Ð½Ð¾Ð²ÑÐµ ÑÐ¾ÑÐºÐ¸
		for(i=0;i<l;i++){
	...
	//  ÐÑÐ»Ð¸ Ð¼Ð°ÑÑÐ¸Ð² Ð±ÑÑÐµÑÐ° Ð¿ÑÑÑ Ð¸ Ð¼Ð°ÑÑÐ¸Ð² ÑÐµÐºÑÑÐµÐ¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ ÑÐ°ÐºÐ¶Ðµ Ð¿ÑÑÑ, ÑÐ¾ Ð¾ÑÑÐ°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ Ð¿Ð¾ÑÐ¾Ðº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	var self = this;
...
InteractiveTask.AnimationController.prototype.playByLabel = function(label){
	...
	for(i=0;i<l;i++){
		...
		if(this.bufferAnimation[i].canGetObject()){
			...
			//  ÐÐ°Ð¿Ð¾Ð¼Ð¸Ð½Ð°ÐµÐ¼ ÑÐ»Ð¾Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹
...
/************************************************************************************************************************/
/************ÐÐ±ÑÐµÐºÑ ÑÐ¾Ð´ÐµÑÐ¶Ð°ÑÐ¸Ð¹ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÐµ ÑÐ¾ÑÑÐ¾ÑÐ½Ð¸Ñ Ð°Ð½Ð¸Ð¼Ð¸ÑÑÐµÐ¼Ð¾Ð³Ð¾ Ð¾Ð±ÑÐµÐºÑÐ° Ð¸ Ð¼ÐµÑÐ¾Ð´Ñ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ñ Ð¾Ð±ÑÐµÐºÑÐ°*******************/
/************************************************************************************************************************/
...
InteractiveTask.AnimationObject.prototype.parseXML = function(){
	//alert("parseXml");
	//  ÐÐ°Ð¿Ð¾Ð¼Ð¸Ð½Ð°ÐµÐ¼ ÑÐ°Ð³ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.step = parseFloat(this.xml["-step"]);
	//  ÐÐ°Ð¿Ð¾Ð¼Ð¸Ð½Ð°ÐµÐ¼ Ð²ÑÐµÐ¼Ñ ÑÐµÑÐµÐ· ÐºÐ¾ÑÐ¾ÑÐ¾Ðµ Ð½ÐµÐ¾Ð±ÑÐ¾Ð´Ð¸Ð¼Ð¾ ÑÑÐ°ÑÑÐ¾Ð²Ð°ÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ
	this.startFrom = parseFloat(this.xml["-startTime"]);
	//  ÐÐ°ÑÐ¸ÐºÐ»Ð¸Ð²Ð°ÑÑ Ð¸Ð»Ð¸ Ð½ÐµÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ
	this.cicling = (this.xml["-cicling"] == "true");
	//  Ð£ÑÑÐ°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÑÑ Ð½Ð°ÑÑÑÐ¾Ð¹ÐºÐ¸ Ð¿ÐµÑÐ²Ð¾Ð¹ ÑÐ¾ÑÐºÐ¸ Ð¸Ð»Ð¸ Ð½ÐµÑ
	var isFirstPointSettings = (this.xml["-firstPoint"] == "true");
	//  Ð£Ð´Ð°Ð»ÑÑÑ Ð»Ð¸ Ð¾Ð±ÑÐµÐºÑ Ð¿Ð¾ Ð·Ð°Ð²ÐµÑÑÐµÐ½Ð¸Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.isRemoveObject = (this.xml["-removeObject"] == "true");
	//  Ð£ÑÑÐ°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ ÑÐ¸ÑÐ»Ð¾ Ð·Ð°Ð¿ÑÐ¾ÑÐ¾Ð² Ð´Ð¾ Ð·Ð°Ð¿ÑÑÐºÐ° Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	if(this.xml["-address"]!="") this.address = parseInt(this.xml["-address"]);
	//  ÐÐ¿ÑÐµÐ´ÐµÐ»ÑÐµÐ¼ Ð½ÐµÐ¾Ð±ÑÐ¾Ð´Ð¸Ð¼ Ð»Ð¸ Ð¼Ð½Ð¾Ð¶ÐµÑÑÐ²ÐµÐ½Ð½ÑÐ¹ Ð·Ð°Ð¿ÑÑÐº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.multiple = (this.xml["-multiple"]=="true");
	//  ÐÐ¿ÑÐµÐ´ÐµÐ»ÑÐµÐ¼ Ð±ÑÐ´ÐµÑ Ð»Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ ÑÑÐ°ÑÐ¸ÑÐµÑÐºÐ¾Ð¹, Ñ.Ðµ. Ð´Ð²Ð¸Ð¶ÐµÐ½Ð¸Ðµ ÐºÐ¾ Ð²ÑÐ¾ÑÐ¾Ð¹ ÑÐ¾ÑÐºÐµ Ñ ÑÐµÐºÑÑÐµÐ¹ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ð¸ Ð¸Ð»Ð¸ Ð²Ð¾Ð·Ð²ÑÐ°Ñ Ð½Ð° Ð¿ÐµÑÐ²ÑÑ ÑÐ¾ÑÐºÑ
	this.animationStatic = (this.xml["-static"]=="true");
	//  Ð¼ÐµÑÐºÐ° Ð´Ð»Ñ Ð·Ð°Ð¿ÑÑÐºÐ° Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.label = this.xml["-label"];

	// Ð¡Ð¾Ð·Ð´Ð°ÐµÐ¼ Ð¼Ð°ÑÑÐ¸Ð² ÐºÐ»ÑÑÐµÐ²ÑÑ ÑÐ¾ÑÐµÐº
	...
	// ÐÑÐ»Ð¸ Ð² Ð¼Ð°ÑÑÐ¸Ð²Ðµ ÐºÐ»ÑÑÐµÐ²ÑÑ ÑÐ¾ÑÐµÐº ÑÑÑÐµÑÑÐ²ÑÑÑ Ð¾ÑÑÐ¸ÑÐ°ÑÐµÐ»ÑÐ½ÑÐµ Ð¿Ð¾Ð²Ð¾ÑÐ¾ÑÑ, ÑÐ¾ Ð´ÐµÐ»Ð°ÐµÐ¼ Ð¸Ñ Ð¿Ð¾Ð»Ð¾Ð¶Ð¸ÑÐµÐ»ÑÐ½ÑÐ¼Ð¸
	var i,l;
	...
	//  ÐÐ°ÑÑÑÐ°Ð¸Ð²Ð°ÐµÐ¼ Ð¾Ð±ÑÐµÐºÑ Ð¿Ð¾ Ð¿ÐµÑÐ²Ð¾Ð¹ ÑÐ¾ÑÐºÐµ, ÐµÑÐ»Ð¸ ÑÑÐ¾ Ð½ÐµÐ¾Ð±ÑÐ¾Ð´Ð¸Ð¼Ð¾
...
//  ÐÐµÑÐ¾Ð´ Ð¿ÑÐ¾ÑÑÐµÑÐ° Ð²ÑÐµÑ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ Ñ ÑÐ°Ð¹Ð¼Ð°ÑÑÐ°Ð¼Ð¸
InteractiveTask.AnimationObject.prototype.middlePointsAnimation = function(){
	...
	//  ÐÑÐ»Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ Ð½Ð°ÑÐ¸Ð½Ð°ÐµÑÑÑ Ñ ÑÐµÐºÑÑÐµÐ¹ ÑÐ¾ÑÐºÐ¸, ÑÐ¾ Ð½Ð°ÑÑÑÐ°Ð¸Ð²Ð°ÐµÐ¼ Ð½Ð¾Ð²ÑÐµ Ð¿Ð¾Ð²Ð¾ÑÐ¾Ñ Ð¸ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ
	...
	var ID = 0;
	//  Ð´ÐµÐ»ÑÑÐ° Ð¿ÐµÑÐµÐ¼ÐµÑÐµÐ½Ð¸Ðµ Ð¾Ð±ÑÐµÐºÑÐ°
	var deltaX, deltaY;
	//  ÐÐµÐ»ÑÑÑ Ð¿Ð¾Ð²Ð¾ÑÐ¾ÑÐ°, Ð¼Ð°ÑÑÑÐ°Ð±Ð¸ÑÐ¾Ð²Ð°Ð½Ð¸Ñ Ð¸ Ð¿ÑÐ¾Ð·ÑÐ°ÑÐ½Ð¾ÑÑÐ¸ ÑÐ¾Ð¾ÑÐ²ÐµÑÑÑÐ²ÐµÐ½Ð½Ð¾
	var deltaR, deltaS, deltaA;
	//  ÐÐµÐ»ÑÑÐ° ÑÐ²ÐµÑÐ° Ð¿Ð¾ ÑÐ¿ÐµÐºÑÑÐ°Ð¼
	var deltaRed, deltaGreen, deltaBlue;
	//	Ð§Ð¸ÑÐ»Ð¾ ÑÐ°ÑÑÐµÐ¹ Ð½Ð° ÐºÐ¾ÑÐ¾ÑÐ¾Ðµ Ð´ÐµÐ»Ð¸ÑÑÑ Ð¾ÑÑÐµÐ·Ð¾Ðº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	var sempleTime = Math.floor(InteractiveTask.CONST.ANIMATION_FRAME_RATE*this.step);

	var i,j,l;
	l = this.keyPoints.length;
	//	Ð²ÑÑÐ¸ÑÐ»ÑÐµÐ¼ Ð½Ð°ÑÐ°Ð»ÑÐ½ÑÐµ Ð·Ð½Ð°ÑÐµÐ½Ð¸Ñ ÑÐ²ÐµÑÐ°
	var r,g,b;
	...
	//  Ð¡Ð¾Ð·Ð´Ð°ÐµÐ¼ Ð¼Ð°ÑÑÐ¸Ð² Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.arrayAnimationPoints = new Array();
	//  ÐÑÐ»Ð¸ Ð½ÑÐ¶Ð½Ð¾ Ð·Ð°Ð¿ÑÑÑÐ¸ÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ ÑÐµÑÐµÐ· Ð½ÐµÐºÐ¾ÑÐ¾ÑÐ¾Ðµ Ð²ÑÐµÐ¼Ñ, ÑÐ¾ ÑÑÐ°Ð²Ð¸Ð¼ Ð² Ð¼Ð°ÑÑÐ¸Ð² Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº Ð¿Ð¾Ð´ÑÐ¾Ð´ÑÑÐµÐµ ÐºÐ¾Ð»Ð¸ÑÐµÑÑÐ²Ð¾
	//  Ð?Ð½Ð°ÑÐµ Ð·Ð°Ð¿Ð¸ÑÑÐ²Ð°ÐµÐ¼ ÑÐ¾Ð»ÑÐºÐ¾ Ð¾Ð´Ð½Ñ Ð¿ÐµÑÐ²ÑÑ ÑÐ¾ÑÐºÑ
	...
	//	ÐÑÐ¾ÑÑÐ¸ÑÑÐ²Ð°ÐµÐ¼ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ð¸ Ð´Ð»Ñ ÐºÐ°Ð¶Ð´Ð¾Ð³Ð¾ Ð¾ÑÑÐµÐ·ÐºÐ°
	var currentRed, currentBlue, currentGreen;
	if(this.step!=0){
		for(i=1;i<l;i++){
			//	ÐÑÐ¾ÑÑÑÑ ÑÐ¼ÐµÑÐµÐ½Ð¸Ð¹ Ð¿Ð¾ Ð¾ÑÑÐ¼ Ð´Ð»Ñ ÑÐµÐºÑÑÐµÐ³Ð¾ Ð¾ÑÑÐµÐ·ÐºÐ°
			...
			//	ÐÑÑÐ¸ÑÐ»ÑÐµÐ¼ ÑÐ¼ÐµÑÐµÐ½Ð¸Ðµ ÐºÐ°Ð¶Ð´Ð¾Ð³Ð¾ ÑÐ¿ÐµÐºÑÑÐ° ÑÐ²ÐµÑÐ°
			if(this.object!=null){
			...
			//	ÐÐ°Ð¿Ð¸ÑÑ Ð² Ð¼Ð°ÑÑÐ¸Ð² Ð²ÑÐµÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¾Ð½Ð½ÑÑ ÑÐ¾ÑÐµÐº
			for(j=0;j<sempleTime;j++){
				//  ÐÐ¿ÑÐµÐ´ÐµÐ»ÑÐµÐ¼ Ð¸Ð½Ð´ÐµÐºÑ Ð½Ð¾Ð²Ð¾Ð¹ ÑÐ¾ÑÐºÐ¸
				ID = this.arrayAnimationPoints.length;
				//  ÐÑÑÐ¸ÑÐ»ÑÐµÐ¼ ÑÐ²ÐµÑ Ð¾Ð±ÑÐµÐºÑÐ° Ð² ÑÐµÐºÑÑÐµÐ¹ ÑÐ¾ÑÐºÐµ
				...
				//  ÐÐ°Ð¿Ð¸ÑÑÐ²Ð°ÐµÐ¼ Ð½Ð¾Ð²ÑÑ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐºÑ
			...
			//  ÐÑÐ»Ð¸ Ð½Ð° ÑÐ»ÐµÐ´ÑÑÑÐµÐ¹ ÑÐ¾ÑÐºÐµ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ Ð½ÑÐ¶ÐµÐ½ ÑÐ°Ð¹Ð¼Ð°ÑÑ, ÑÐ¾ Ð·Ð°Ð¿Ð¸ÑÑÐ²Ð°ÐµÐ¼ ÑÑÐ¾Ð»ÑÐºÐ¾ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº ÑÐ°Ð²Ð½ÑÑ ÑÐ»ÐµÐ´ÑÑÑÐµÐ¹, ÑÐºÐ¾Ð»ÑÐºÐ¾ Ð½ÐµÐ¾Ð±ÑÐ¾Ð´Ð¸Ð¼Ð¾
	...
	//	Ð£Ð´Ð°Ð»ÑÐµÐ¼ Ð¿ÐµÑÐ²ÑÑ ÑÐ¾ÑÐºÑ Ð´Ð»Ñ Ð±Ð¾Ð»ÐµÐµ Ð¿Ð»Ð°Ð²Ð½Ð¾Ð³Ð¾ Ð¿ÐµÑÐµÑÐ¾Ð´Ð° Ð¿ÑÐ¸ Ð·Ð°ÑÐ¸ÐºÐ»Ð¸Ð²Ð°Ð½Ð¸Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.arrayAnimationPoints.shift();
};

//  Ð¤ÑÐ½ÐºÑÐ¸Ñ Ð¿ÐµÑÐµÑÐ¾Ð´Ð° Ðº ÑÐ»ÐµÐ´ÑÑÑÐµÐ¹ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½Ð¾Ð¹ ÑÐ¾ÑÐºÐµ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
InteractiveTask.AnimationObject.prototype.gotoNextPoint = function(){
	//  ÐÐµÑÐ²ÑÑ ÑÐ¾ÑÐºÑ Ð¸Ð· Ð¼Ð°ÑÑÐ¸Ð²Ð° Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº, Ð·Ð°Ð¿Ð¾Ð¼Ð¸Ð½Ð°ÐµÐ¼ ÐµÑ Ð¸ ÑÐ´Ð°Ð»ÑÐµÐ¼ Ð¸Ð· Ð¼Ð°ÑÑÐ¸Ð²Ð°
	var currentPoint = this.arrayAnimationPoints.shift();
	//  ÐÐµÑÐµÐ½Ð¾ÑÐ¸Ð¼, Ð¿Ð¾Ð²Ð¾ÑÐ°ÑÐ¸Ð²Ð°ÐµÐ¼, Ð¼Ð°ÑÑÑÐ°Ð±Ð¸ÑÑÐµÐ¼ Ð¸ Ð¸Ð·Ð¼ÐµÐ½ÑÐµÐ¼ Ð¿ÑÐ¾Ð·ÑÐ°ÑÐ½Ð¾ÑÑÑ Ð¾Ð±ÑÐµÐºÑÐ°
	...
	//  Ð?Ð·Ð¼ÐµÐ½ÑÐµÐ¼ ÑÐ²ÐµÑ Ð¾Ð±ÑÐµÐºÑÐ°
	...
	//  ÐÑÐ»Ð¸ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ Ð·Ð°ÑÐ¸ÐºÐ»Ð¸Ð½Ð°, ÑÐ¾ ÑÑÐ°Ð²Ð¸Ð¼ Ð¿ÐµÑÐ²ÑÑ ÑÐ¾ÑÐºÑ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½Ð¾Ð¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ Ð² ÐºÐ¾Ð½ÐµÑ Ð¼Ð°ÑÑÐ¸Ð²Ð°, Ð¸Ð½Ð°ÑÐµ Ð¾Ð½Ð° Ð¾ÑÑÐ°ÐµÑÑÑ ÑÐ´Ð°Ð»ÐµÐ½Ð½Ð¾Ð¹
	if(this.cicling){
...
//  ÐÑÐ¾Ð²ÐµÑÐºÐ° Ð½Ð° Ð·Ð°Ð²ÐµÑÑÐµÐ½Ð¸Ðµ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ (Ð² ÑÐ»ÑÑÐ°Ðµ ÐµÑÐ»Ð¸ Ð¼Ð°ÑÑÐ¸Ð² Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ ÑÐ¾ÑÐµÐº Ð¿ÑÑÑÐ¾Ð¹)
InteractiveTask.AnimationObject.prototype.isComplate = function(){
	return this.arrayAnimationPoints.length==0;
};
//  ÐÑÐ¾Ð²ÐµÑÐºÐ°, Ð¼Ð¾Ð¶Ð½Ð¾ Ð»Ð¸ Ð·Ð°Ð¿ÑÑÑÐ¸ÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ (Ð¸ÑÑÐµÑÐ¿Ð°Ð½ Ð»Ð¸ Ð»Ð¸Ð¼Ð¸Ñ Ð·Ð°Ð¿ÑÐ¾ÑÐ¾Ð² Ðº Ð½ÐµÐ¹)
InteractiveTask.AnimationObject.prototype.canGetObject = function(){
	if(this.address==0) return true;
	--this.address;
	return false;
};
//  ÐÑÐ¾Ð²ÐµÑÐºÐ° Ð°Ð²ÑÐ¾Ð¼Ð°ÑÐ¸ÑÐµÑÐºÐ¸ Ð»Ð¸ Ð·Ð°Ð¿ÑÑÐºÐ°ÐµÑÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ñ
...
//  ÐÐ°Ð¿Ð¸ÑÑ ÐºÐ»ÑÑÐµÐ²ÑÑ ÑÐ¾ÑÐµÐº Ð² Ð¼Ð°ÑÑÐ¸Ð² Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÑ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¹
...
/************************************************************************************************************************/
/************ ÐÐ±ÑÐµÐºÑ ÑÐ¾Ð´ÐµÑÐ¶Ð°ÑÐ¸Ð¹ Ð¿ÑÐ¾Ð¼ÐµÐ¶ÑÑÐ¾ÑÐ½ÑÐµ ÑÐ¾ÑÑÐ¾ÑÐ½Ð¸Ñ ÑÐ»Ð¾Ð¶Ð½Ð¾Ð¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸ Ð¸Ð· ÐºÐ°ÑÑÐ¸Ð½Ð¾Ðº ************************************/
/************************************************************************************************************************/
/**
 * @param options ÑÐ»Ð¾Ð¶Ð½ÑÐ¹ Ð¾Ð±ÑÐµÐºÑ:
 *      - xml - ÐºÐ¾Ð½ÑÐ¸Ð³ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
 *      - class - ÑÐ¾Ð´Ð¸ÑÐµÐ»ÑÑÐºÐ¸Ð¹ ÐºÐ»Ð°ÑÑ Ð°Ð½Ð¸Ð¼Ð¸ÑÑÐµÐ¼Ð¾Ð³Ð¾ Ð¾Ð±ÑÐµÐºÑÐ°  ???? - Ð½Ð°Ð²ÐµÑÐ½Ð¾Ðµ Ð½Ðµ Ð½ÑÐ¶ÐµÐ½
 *      - object - Ð°Ð½Ð¸Ð¼Ð¸ÑÑÐµÐ¼ÑÐ¹ Ð¾ÑÑÐµÐºÑ (Konva.Sprite)
 * @constructor
 */
...
InteractiveTask.AnimationSprite.prototype.parseXML = function(){
	...
	//  Ð¼ÐµÑÐºÐ° Ð´Ð»Ñ Ð·Ð°Ð¿ÑÑÐºÐ° Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
	this.label = this.xml["-label"];
	//  Ð?Ð¼Ñ ÑÐµÐºÑÑÐµÐ¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸  animation : 'standing'
...
InteractiveTask.TestChangeFrame = function(width, height, controller){
	...
	var successBackground = new Konva.Rect({
		width : this.width,
		height : this.height,
		fill : InteractiveTask.formatColorFlashToCanvasRGBA("0x999999", 0),
		stroke : InteractiveTask.formatColorFlashToCanvasRGBA("0x0fa507", 1),
		strokeWidth : 5
	});
	...
	var failBackground = new Konva.Rect({
		...
		height : this.height,
		fill : InteractiveTask.formatColorFlashToCanvasRGBA("0x999999", 0),
		stroke : InteractiveTask.formatColorFlashToCanvasRGBA("0xbd3030", 1),
		strokeWidth : 5
	});
...
/**
 * ôóíêöèÿ äëÿ óñòàíîâêè âðàùåíèÿ è ïåðåìåùåíèÿ òàíîâ (ïîëüçîâàòåëüñêèõ è öâåòíûõ)
 * @param self - îáúåêò òàíà êîòîðûé äîëæåí ïåðåìåùàòüñÿ
 * @param event - ñîáûòèå íàæàòèÿ íà òàí äëÿ îïðåäåëåíèÿ ñåíñîðíîãî ñîáûòèÿ
 */
 ...
 * @param target
 * @param options = {
 *    isRotate : boolean,           - Íóæíîëè ïîâîðà÷èâàòü îáúåêò
 *    isDrag : boolean,             - Íóæíî ëè ïåðåìåùàòü îáúåêò
 *    layer : Konva.Layer,          - Ñëîé â êîòîðîì íàõîäèòñÿ îáúåêò äî ýòîé ôóíêöèè
 *    callback() : Function         - Ôóíêöèÿ, êîòîðóþ íóæíî âûçâàòü ïîñëå çàâåðùåíèÿ ðàáîòû ñ îáúåêòîì
 *    rotate(degree) : Function     - Ôóíêöèÿ âûïîëíÿåìàÿ ïðè ýëåìåíòàðíîì ïîâîðîòå
 * }
 */
...
/**
 *
 * @param time - ñòðîêîâîå ïðåäñòàâëåíèå âðåìåíè âñåãî ïÿòü ñèìâîëîâ
 *  Ïåðâûå äâà - ìèíóòû
 *  Ïîñëåäíèå äâà - ñåêóíäû
 *  Â öåíòðå - ðàçäåëèòåëü
 */
...
/**
 * Ôóíêöèÿ âîñïðîèçâåäåíèÿ âíåøíåãî çâóêà (çàâåðøåíèå/íà÷àëî çàäàíèÿ)
 * @param player - îáúåêò ïëååðà â êîòîðîì âûçûâàåòñÿ ïðîäîëæåíèå çàäàíèÿ
 */
InteractiveTask.audioControl = function(player){
	...
	var playAudio;
	/**
	 * Èíèöèàëèçàöèÿ íîâîãî àóäèîôàéëà
	 * @param src - ññûëêà íà ôàéë   InteractiveTask.AUDIO.init(InteractiveTask.PATH+"audio/"+this.getStartAudioID(this.currentTaskID)+".wav", false);
	 * @param isDispatch - ïðîèçâîäèòü ëè ïðîäîëæåíèå çàäàíèé ïîñëå ïðîèãðûâàíèÿ çâóêà
	 */
